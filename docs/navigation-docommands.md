...
| `navigate_route` | **`name`** *or* **`waypoints`**; optional `start_index`, `wait` | `{ "status": "navigating", "motion": "route", ... }` |
...
| `wait` | If `true`, block until the whole route finishes (default `false`) |

`get_status` includes a `route` blob (`state`, `name`, `index`, `total`, `location`, `waypoints`, `error_msg`). While a route is active, top-level `motion` is `"route"`.

`suspend` remembers the current leg; `resume` restarts the route from that index.
