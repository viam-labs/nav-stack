...
| `nav_loc_refine_on_disagree` | Nav (`builtin`) | When the current lidar scan is a poor explanation of the map at the published pose, stop, run a local `check_localization`, then resume or fail the goal as `localization_lost` (default `true`). Uses scan-vs-map residual, not SLAM match score. Does **not** enable `periodic_relocalize_during_navigation` |
